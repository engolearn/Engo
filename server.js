...
// جلب التنبيه النشط الحالي (للواجهة الرئيسية)
    ...
    try {
        ...
        res.json({ 
            ...
            announcement: {
                ...
                hasCountdown: announcement.hasCountdown,
...
// إنشاء تنبيه جديد (للأدمن)
app.post('/api/admin/announcements', auth, adminAuth, async (req, res) => {
    try {
        const { text, hasCountdown, countdownDate, expiryDate } = req.body;
        
        if (!text || text.trim() === '') {
            return res.status(400).json({ message: 'نص التنبيه مطلوب' });
        }
        
        // تعطيل جميع التنبيهات النشطة أولاً
        ...
        res.json({ success: true, message: 'تم نشر التنبيه بنجاح', announcement });
    } catch (error) {
...
app.put('/api/admin/announcements/:id', auth, adminAuth, async (req, res) => {
    try {
        const { id } = req.params;
        const { text, hasCountdown, countdownDate, expiryDate, isActive } = req.body;
        ...
        if (hasCountdown !== undefined) announcement.hasCountdown = hasCountdown;
        if (countdownDate !== undefined) announcement.countdownDate = countdownDate;
        ...
        res.json({ success: true, message: 'تم تحديث التنبيه بنجاح', announcement });
    } catch (error) {
